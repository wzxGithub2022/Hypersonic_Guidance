...
alpha_lim_l = deg2rad(-10);
alpha_lim_u = deg2rad(10);

state_lim = [v_lim_l,v_lim_u,gama_lim_l,gama_lim_u,alpha_lim_l,alpha_lim_u];
...
%------------------------若代码一直在循环就是卡在这个区间，sos问题一直得不到解决-----------------------%
    while (~solved) 
        ...
        [sos_prob,~,~] = Opt_Model_e1(n,state_lim,cond_u,lambda,ccm_eps,return_metric);
        if (sos_prob == 0)
            solved = 1;
            fprintf('feasible \n');
        else
...
%---------------------------------------------------------------------------------------------------%
    ...
    while(cond_u - cond_l >= eps)     %误差较大，细分优化
        ...
        [sos_prob, w_lower, w_upper] = Opt_Model_e1(n,state_lim,condn,lambda,ccm_eps,return_metric);
        if (sos_prob == 0)
...
save_name = 'metric_Model_e1_vectorized.mat';          %之后的部分只调用了一次CCM_OTP，还用save_name把东西取出来，之前打掉的部分已经完成了优化CCM的功能
%%
[sos_prob, w_lower, w_upper] = Opt_Model_e1(n,state_lim,condn,lambda,ccm_eps,return_metric,save_name);      %在这一步存好，sos_prob:sum of squares problem
...
f1 = -v * cos(gama);
f2 = v * sin(gama);
f3 = - Cd/(m*g) * alpha^2 - sin(gama);
f4 = CL/(m*g) * alpha / v - cos(gama)/v;
f5 = 0;

f_mat(v, gama, alpha) = [f1;f2;f3;f4;f5];               %动力学，f存在的意义是做 偏W/偏f 那一项
...
df3_v = 0;
df3_gama = -cos(gama);
df3_alpha = - Cd/(m*g) * 2 * alpha;

df3 = [df3_y, df3_h, df3_v, df3_gama, df3_alpha];

df4_y = 0;
df4_h = 0;
df4_v = - CL/(m*g) * alpha / v^2 + cos(gama) / v^2;
df4_gama = sin(gama) / v;
df4_alpha = CL/(m*g) / v;
...
df_mat(v, gama, alpha) = [df1;df2;df3;df4;df5];      %df都用得上，5*5没毛病

%------------------------END：动力学和微分动力学---------------------------%
%%
B = [zeros(4,1);1];       % 5*1 的double
            
Bw_symfnc(v, gama, alpha) = [zeros(2,2);         %气动系数上的扰动引起的Tube就大得离谱
                             0.01*Cd/(m*g) * alpha^2, 0;
                             0, 0.01*CL/(m*g) * alpha / v;
                             0,0];     %扰动是0.1倍的气动系数误差
...
alpha_range = linspace(alpha_lim_l, alpha_lim_u, ctrl_N);

delta_u = zeros(ctrl_N,ctrl_N,ctrl_N);       %是3维数组，因为有3个状态
eig_CCM_min = zeros(ctrl_N, ctrl_N, ctrl_N);
eig_CCM_max = zeros(ctrl_N, ctrl_N, ctrl_N);
eig_W = zeros(ctrl_N,ctrl_N,2);                     %W只对i,j,k做，因为你的u会影响攻角，所以期望W只是高度、速度、弹道倾角的函数
...
for i = 1:length(v_range)
    ...
    for j = 1:length(gama_range)
        ...
        for k = 1:length(alpha_range)
                ...
                x = [0;0;v_range(i);gama_range(j);alpha_range(k)];    %射程随机，飞行状态遍历
                
                Bw = double( Bw_symfnc(x(3),x(4),x(5)) );
                ...
                f = double( f_mat(x(3),x(4),x(5)) );           %状态微分
                df = double( df_mat(x(3),x(4),x(5)) );         %状态切向微分
                
                % W是对称的，F也是对称的，本质是G（x），F需要负定，这里的对应关系好像有问题？
                F = - W_eval(dw_poly_v_fnc(x)) * f(3) - W_eval(dw_poly_gama_fnc(x)) * f(4)...      
                ...
                eig_CCM_min(i,j,k) = min(eig(R_CCM));     %若他为负值，则不满足收缩条件
