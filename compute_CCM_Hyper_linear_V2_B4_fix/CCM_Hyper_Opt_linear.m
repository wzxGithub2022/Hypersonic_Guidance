...
% 微分系数矩阵理解得有问题，df44和df_perp要改

function [solved,w_lower,w_upper] = CCM_Hyper_Opt_linear ...
                (n,h_lim,v_lim,gama_lim,df_full, ...
...
%dynamics f，动力学
f = df_full * x;             %动力学，4*1，含x，这里的f得和x保持同样的维度

%基于微分动力学的df_mat的前2行，控制会影响到后2行，为了和B_perp配合
df44 = df_full(:,:);          %已改标注
df_perp = df44(1:2,:);        %微分动力学，2*4，含x，这里的df得和B_perp配合
...
if (return_metric)
    if (solved==0)
        ...
        for i = 1:length(w_poly)
            W_sol(:,:,i) = clean(double(SOS_soln.eval(W_list{i})),1e-3); %clean Remove terms with small coefficients，1e-3是不是清理得太多了，OMG一动百动，control的问题在这里得到了解决
        ...
        W_upper = clean(double(SOS_soln.eval(W_upper)),1e-3);   %clean问题标注
