...
if (return_metric)
    if (solved==0)
        ...
        for i = 1:length(w_poly)
            W_sol(:,:,i) = clean(double(SOS_soln.eval(W_list{i})),clean_epsilon); %clean Remove terms with small coefficients
        ...
        W_upper = clean(double(SOS_soln.eval(W_upper)),clean_epsilon);
