...
function [solved,w_lower,w_upper] = ...
    CCM_Hyper_Opt(n,g,h_lim,v_lim,gama_lim,alpha_lim,state_base,...
...
%dynamics f，动力学
S   = 0.5026;                       %参考面积
% g = 9.81;                         %调试程序用，实则g是function的输入参数
% rou = 1.225 * exp(-x(2)/7110);    %密度rou，(msspoly不能计算指数，泰勒展开逼近)
hf  = -x(2)/7110;                   %h fake，中间变量
rou = 1.225 * (1 + hf + hf^2/2 + hf^3/6 + hf^4/24 + hf^5/120 + hf^6/720 + hf^7/5040 + hf^8/40320 + hf^9/362880 + hf^10/3628800);
q   = 0.5 * rou * x(3)^2;           %动压
qf  = 0.5 * rou * x(3);             %q fake，伪动压，已约减速度v
M   = x(3) / 340;                   %马赫数
m   = 1600;                         %质量

CL  = 0.4172 + 19.41*x(5) + 10.17*x(5)^2 - M*(0.1004 + 0.7536*x(5));
L   = q*CL*S;                       %升力
Lf  = qf*CL*S;                      %L fake，伪升力，已约减速度v

Cd  = 0.3042 + 0.02988*CL^2;
D   = q*Cd*S;                       %阻力

%三阶切比雪夫逼近1/V，区间[1000,2000]
V_division = 0.0028498843 - 2.984391e-06*x(3) + 1.3615968e-09*x(3)^2 - 2.285664e-13*x(3)^3;
...
box_lim = [ h_lim^2 - (x(2)-h0)^2;
            v_lim^2 - (x(3)-v0)^2;
            gama_lim^2 - (x(4)-gama0)^2;
            alpha_lim^2 - (x(5)-alpha0)^2];       %限定了状态范围
