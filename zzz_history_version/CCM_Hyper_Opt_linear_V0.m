...
% 也就是说你线性化之后系统都换了，此版本留存
function [solved,w_lower,w_upper] = ...
    CCM_Hyper_Opt_linear(n,h_lim,v_lim,gama_lim,alpha_lim,state_base,...
                    f_full,df_full,ddf_full,condn,lambda,ccm_eps,return_metric,varargin)
...
df_perp = [df1;df2;df3;df4];        %微分动力学，4*5，含x，这里的df得和B_perp配合
...
if (return_metric)
    if (solved==0)
        ...
        for i = 1:length(w_poly)
            W_sol(:,:,i) = clean(double(SOS_soln.eval(W_list{i})),1e-3); %clean Remove terms with small coefficients，1e-3是不是清理得太多了
