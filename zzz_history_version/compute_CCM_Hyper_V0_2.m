...
sin_x_cheby = @(xx) 0.985400513847416*xx - 0.142496853019355*xx^3;
cos_x_cheby = @(xx) 0.999396553656190 - 0.495559134405119*xx^2 + 0.036782872656059*xx^4;
...
%dynamics f，动力学
S   = @(x) 0.5026;                       %参考面积
hf  = @(x) x(2)*R0/7110;                 %h fake，中间变量
rou = @(x) 1.225 * (1.124508748184077 + 0.459262515874491*hf + 3.155718757366057*hf^2 + 1.426742501850045*hf^3 + 0.374836249394692*hf^4);
q   = @(x) 0.5 * rou * (x(3)*sqrt(R0*g))^2;           %动压
qf  = @(x) 0.5 * rou * x(3)*sqrt(R0*g);               %q fake，伪动压，已约减速度v
M   = @(x) x(3)*sqrt(R0*g) / 340;                     %马赫数
m   = @(x) 1600;                                      %质量

CL = @(x) 0.4172 + 19.41*x(5) + 10.17*x(5)^2 - M*(0.1004 + 0.7536*x(5));
L_nor = @(x) q*CL*S / (m*g);                        %升力
Lf_nor = @(x) qf*CL*S / (m*g);                      %L fake，伪升力，已约减速度v

Cd = @(x) 0.3042 + 0.02988*CL^2;
D_nor = @(x) q*Cd*S / (m*g);                        %阻力

%切比雪夫逼近1/V，对归一化之后的V，区间[3,7]
V_division = @(x) 1.101028089323823 - 0.473922941895471*x(3) + 0.099720451270527*x(3)^2 - 0.010265506659430*x(3)^3 + 4.140771839924636e-04*x(3)^4;

f1 = @(x) -x(3) * cos_a;                 %不是很必要，先留存在这里
f2 = @(x) x(3) * sin_a;
f3 = @(x) -D_nor - sin_g;
f4 = @(x) Lf_nor - cos_g*V_division;
f5 = @(x) 0;
