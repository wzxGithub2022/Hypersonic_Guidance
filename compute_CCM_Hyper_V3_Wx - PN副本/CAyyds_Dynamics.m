...
if feedback
    miss_distance = [X(1)-yd,X(2)-hd]*R0;
    if norm(miss_distance) > -10
        u_d = 0;     %标称的反馈，没有的，直接就是让攻角程序化地改变
        % [condn] = CAyyds_find_condn(n,X_d,condn_prev,lambda,ccm_eps);
        condn = condn_prev;
        save_name = 'Hyper_Wx_rule.mat';
        [solved,w_lower,w_upper,W_upper] = CAyyds_CCM_Opt(n,X_d,condn,lambda,ccm_eps,return_metric,save_name);
        load(save_name);
        W = W_eval(w_poly_fnc(X));  %由当前状态X执行W的生成规则得到W
